#!/usr/bin/env python3
"""Run right 180 turn (pulse+settle) while logging IMU gz and integrated progress.
...
def main() -> int:
    ...
    strong_speed, strong_ms = 0.08, 80
    weak_speed, weak_ms = 0.06, 50
    ...
    out_csv = Path("/home/fukuhala/python_ws/dmc_ai_mobility/right180_imu_log.csv")
    out_png = Path("/home/fukuhala/python_ws/dmc_ai_mobility/right180_imu_plot.png")
    ...
    # t, gz_raw, gz_dir, gz_med, progressed
    ...
    try:
        while True:
            ...
            while time.monotonic() < settle_deadline:
                ...
                t_rel = now - t_start
                rows.append((t_rel, float(gz_raw), float(gz_dir), float(gz_med), float(progressed)))
    ...
    with out_csv.open("w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["t_s", "gz_raw", "gz_dir", "gz_med", "progress_deg"])
        w.writerows(rows)
    ...
    ax[1].plot(ts, prog, label="progress_deg", linewidth=2)
    ...
    fig.suptitle("Right 180 turn IMU log (gz) + integrated progress")
    fig.tight_layout()
