#!/usr/bin/env python3
"""Run left 360 turn (pulse+settle) while logging IMU gz and integrated progress.

We keep a large but finite max_pulses for safety.
Left progress is treated as positive by using gz_dir = -gz_raw.
...
def main() -> int:
    ...
    # Left is asymmetric: use stronger pulses.
    strong_speed, strong_ms = 0.22, 180
    weak_speed, weak_ms = 0.15, 120
    strong_until_deg = 200.0
    ...
    out_csv = Path("/home/fukuhala/python_ws/dmc_ai_mobility/left360_imu_log.csv")
    out_png = Path("/home/fukuhala/python_ws/dmc_ai_mobility/left360_imu_plot.png")
    ...
    try:
        while True:
            ...
            while time.monotonic() < settle_deadline:
                ...
                gz_dir = -clamp(float(gz_raw), -gz_max, +gz_max)  # left positive
    ...
    fig.suptitle("Left 360 turn IMU log (gz) + integrated progress")
    fig.tight_layout()
